# ==========================================================
# VIETTOOLBOX: BÁC SĨ WINDOWS (WPF - V63.9 TURBO SPEED)
# Tác giả: Tuấn Kỹ Thuật Máy Tính
# Tối ưu: Ép ưu tiên CPU High Priority để chạy nhanh hơn
# ==========================================================
...
$LogicBacSiWindows_Turbo = {
    $script:StopWatch = [System.Diagnostics.Stopwatch]::New()
    $script:IsRunning = $false
    $script:CurrentStep = 0 
    $script:SubProc = $null
    $script:LogContent = @("==========================================================","VIETTOOLBOX - BÁO CÁO SỬA LỖI WINDOWS (TURBO)","Ngày: $(Get-Date -Format 'dd/MM/yyyy HH:mm:ss')","==========================================================")
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Window.Resources>
        ...
        <Style TargetType="Button"><Setter Property="Cursor" Value="Hand"/><Setter Property="FontWeight" Value="Bold"/><Setter Property="Foreground" Value="White"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="Button"><Border Background="{TemplateBinding Background}" CornerRadius="8"><ContentPresenter HorizontalAlignment="Center" VerticalAlignment="Center"/></Border></ControlTemplate></Setter.Value></Setter></Style>
    </Window.Resources>
    ...
    <Border Background="#2D2D2D" CornerRadius="12" BorderBrush="#3F3F3F" BorderThickness="1">
        <Grid>
            ...
            <Grid Name="TitleBar" Grid.Row="0" Background="#252526">
                ...
                <TextBlock Text="🩺 VietToolbox: Bác Sĩ Windows (Turbo Mode)" Foreground="Gray" VerticalAlignment="Center" Margin="15,0,0,0" FontSize="11"/>
            ...
            <Grid Grid.Row="1" Margin="25,10,25,25">
                ...
                <StackPanel Grid.Row="1" Margin="0,0,0,20">
                    ...
                    <TextBlock Name="txtDetail" Text="Đã tối ưu ưu tiên CPU High Priority để chạy nhanh hơn." Foreground="Gray" FontSize="12" FontStyle="Italic" Margin="0,8,0,0" HorizontalAlignment="Center" TextAlignment="Center" TextWrapping="Wrap"/>
                </StackPanel>
...
"@
    ...
    $btnClose.Add_Click({
        if ($script:IsRunning) {
            if ([System.Windows.MessageBox]::Show("Tiến trình đang chạy, muốn thoát không?", "Xác nhận", [System.Windows.MessageBoxButton]::YesNo) -eq "No") { return }
    ...
    $MainTimer.Add_Tick({
        ...
        if ($script:IsRunning -and $null -ne $script:SubProc) {
            if ($script:SubProc.HasExited) {
                ...
                $script:SubProc = $null
            } elseif ($pgBar.Value -lt 99) { $pgBar.Value += 0.1 } # Tăng tốc độ thanh tiến trình ảo
        }

        if ($script:IsRunning -and $null -eq $script:SubProc) {
            switch ($script:CurrentStep) {
                2 { 
                    $txtStatusIcon.Visibility = "Collapsed"; $iconRepair.Visibility = "Visible"
                    $txtStatus.Text = "🚀 Sửa lỗi Tầng Offline (High Priority)..."
                    $script:SubProc = Start-Process "dism.exe" -ArgumentList "/Online /Cleanup-Image /RestoreHealth /LimitAccess" -WindowStyle Hidden -PassThru 
                    try { $script:SubProc.PriorityClass = "High" } catch {} # ÉP ƯU TIÊN CPU
                }
                3 { 
                    $txtStatus.Text = "⚠️ Sửa lỗi Tầng Online (High Priority)..."
                    $script:SubProc = Start-Process "dism.exe" -ArgumentList "/Online /Cleanup-Image /RestoreHealth" -WindowStyle Hidden -PassThru 
                    try { $script:SubProc.PriorityClass = "High" } catch {} # ÉP ƯU TIÊN CPU
                }
                4 { 
                    $txtStatus.Text = "🔍 Chốt hạ file hệ thống (High Priority)..."
                    $script:SubProc = Start-Process "sfc.exe" -ArgumentList "/scannow" -WindowStyle Hidden -PassThru 
                    try { $script:SubProc.PriorityClass = "High" } catch {} # ÉP ƯU TIÊN CPU
                }
                5 {
                    ...
                    [System.Windows.MessageBox]::Show("Xong! Báo cáo ở Desktop.", "Thành công")
    ...
    $btnStart.Add_Click({
        if ([System.Windows.MessageBox]::Show("Bắt đầu đại tu Windows Turbo Mode?", "Xác nhận", [System.Windows.MessageBoxButton]::YesNo) -eq "Yes") {
            $script:IsRunning = $true; $script:CurrentStep = 2; $script:StopWatch.Restart(); $MainTimer.Start()
