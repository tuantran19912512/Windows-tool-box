...
$LogicCaiOfficeV148 = {
    ...
    $form.Text = "VIETTOOLBOX - OFFICE V148 (HỖ TRỢ OFFICE TOOL PLUS)"; $form.Size = "820,860"; $form.BackColor = "White"; $form.StartPosition = "CenterScreen"
    ...
    # --- HÀM TẢI DUAL-ENGINE (API + WEB CRAWL VƯỢT RÀO) ---
    function Download-Core ($InputSource, $DestPath) {
        $HttpClient = New-Object System.Net.Http.HttpClient
        # Bọc áo khoác giả làm trình duyệt Chrome xịn
        ...
        try {
            ...
            # --- ENGINE 2: NẾU API XỊT (404/403), DÙNG WEB CRAWL ---
            if (-not $response.IsSuccessStatusCode) {
                ...
                # Nếu Google bắt xác nhận "File quá lớn không quét được virus", lấy Token để ép tải
                if ($response.IsSuccessStatusCode -and $response.Content.Headers.ContentType.MediaType -match "text/html") {
                    ...
                    if ($html -match 'confirm=([a-zA-Z0-9\-_]+)') {
                        ...
                        [System.Windows.Forms.MessageBox]::Show("Google Drive đã khóa file này (Không cho tải công khai)!", "Lỗi Google Drive")
                        return $false
                    }
                }
            }

            # Chốt sổ: Nếu cả 2 Engine đều xịt, bung bảng báo lỗi cho anh em biết
            if (-not $response.IsSuccessStatusCode) {
                $err = $response.Content.ReadAsStringAsync().Result
                [System.Windows.Forms.MessageBox]::Show("Cả API và Web đều bị chặn!`n`nMã lỗi: $($response.StatusCode)`nChi tiết: $err", "Lỗi tải file")
                return $false
            }
            ...
            [System.Windows.Forms.MessageBox]::Show("Lỗi kết nối mạng: $($_.Exception.Message)", "Lỗi Kỹ Thuật")
            return $false 
        }
    }

    # --- SỰ KIỆN CÀI ĐẶT (ĐÃ THÊM LOGIC TÌM OFFICE TOOL PLUS) ---
    $btnInstall.Add_Click({
        ...
        foreach ($item in $items) {
            ...
            if ($res -eq "CANCELLED") {
                ...
                if ($letter) {
                    # ƯU TIÊN TÌM OFFICE TOOL PLUS TRƯỚC
                    $otpPath = "$($letter):\Office Tool Plus.exe"
                    $setupPath = "$($letter):\setup.exe"
                    
                    if (Test-Path $otpPath) {
                        $item.SubItems[1].Text = "💿 Đang chạy Office Tool Plus..."; [System.Windows.Forms.Application]::DoEvents()
                        Start-Process $otpPath -Wait
                    } elseif (Test-Path $setupPath) {
                        Start-Process $setupPath -Wait
                    } else {
                        [System.Windows.Forms.MessageBox]::Show("Không tìm thấy file cài đặt nào hợp lệ (setup.exe hoặc Office Tool Plus.exe) trong file ISO này!", "Lỗi Cài Đặt")
                    }
