...
$btnStart.Add_Click({
    ...
    try {
        # Trich xuat lay rieng con so Index (Vi du tu "1 - Windows 10 Pro" -> lay so "1")
        $selectedIndexNumber = $cmbIndex.SelectedItem.ToString().Split('-')[0].Trim()
        $wimPath = $txtWim.Text

        $lblStatus.Text = "Trang thai: [10%] Dang lay quyen WinRE..."
        ...
        Start-Process -FilePath "dism.exe" -ArgumentList "/Mount-Image /ImageFile:$winrePath /Index:1 /MountDir:$mountDir" -Wait -WindowStyle Hidden

        $lblStatus.Text = "Trang thai: [60%] Dang tich hop cau lenh..."
...
echo.
echo [3/3] Dang cap nhat bo khoi dong he thong...
...
"@
        ...
        Update-UI
        Start-Process -FilePath "dism.exe" -ArgumentList "/Unmount-Image /MountDir:$mountDir /Commit" -Wait -WindowStyle Hidden
        ...
        [System.Windows.Forms.MessageBox]::Show("Qua trinh chuan bi hoan tat! May tinh se khoi dong lai ngay bay gio de cai dat.", "Thanh cong", "OK", "Information")
