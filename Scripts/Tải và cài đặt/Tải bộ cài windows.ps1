# ==========================================================
# VIETTOOLBOX ISO CLIENT V139 - WPF EDITION (ETA CALCULATION)
# ==========================================================
...
$LogicIsoClientV139 = {
    ...
    $MaGiaoDien = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" Title="VIETTOOLBOX V139 - ETA DOWNLOADER" Width="880" Height="820" WindowStartupLocation="CenterScreen" Background="#F4F7F9" FontFamily="Segoe UI">
    <Grid Margin="20">
        ...
        <StackPanel Grid.Row="0" Margin="0,0,0,15">
            <TextBlock Text="TRUNG TÂM TẢI XUỐNG V139" FontSize="24" FontWeight="Bold" Foreground="#1A237E"/>
            <TextBlock Text="Tự động tính toán thời gian hoàn tất (ETA)" Foreground="#666666"/>
        </StackPanel>
        ...
        <Grid Grid.Row="5">
            ...
            <Button Name="btnSync" Grid.Column="0" Content="🔄 LÀM MỚI" Height="60" Background="#455A64" Foreground="White"/><Button Name="btnDownload" Grid.Column="2" Content="🚀 BẮT ĐẦU TẢI" Height="60" Background="#007ACC" Foreground="White" FontSize="18" FontWeight="Bold"/>
        </Grid>
    </Grid>
</Window>
"@
    ...
    function Load-IsoList {
        try {
            ...
            $lblStatus.Text = "✅ Đã tải xong danh sách."
            ...
            try {
                ...
                while (($bytesRead = $stream.Read($buffer, 0, $buffer.Length)) -gt 0) {
                    ...
                    if ($sw.ElapsedMilliseconds -ge 800) {
                        ...
                        # --- CÔNG THỨC TÍNH THỜI GIAN (ETA) ---
                        if ($totalBytes -gt 0 -and $tocDoByte -gt 0) {
                            $conLaiByte = $totalBytes - $totalRead
                            $giayConLai = $conLaiByte / $tocDoByte
                            $ts = [TimeSpan]::FromSeconds($giayConLai)
                            if ($ts.TotalHours -ge 1) {
                                $lblTime.Text = "⏳ Còn lại: $([int]$ts.TotalHours)g $($ts.Minutes)p"
                            } else {
                                $lblTime.Text = "⏳ Còn lại: $($ts.Minutes)p $($ts.Seconds)s"
                            }
                        }
