# ==========================================================
# ISO CLIENT V137 - AUTO DETECT EXTENSION (WIM, ISO, ZIP, CAB)
# ==========================================================
Add-Type -AssemblyName System.Windows.Forms, System.Drawing
Add-Type -AssemblyName System.Net.Http

# Giải mã API Key (Giữ nguyên của ông)
...
$LogicIsoClientV137 = {
    ...
    $form.Text = "VIETTOOLBOX - ISO V137 (AUTO EXTENSION)"; $form.Size = "880,920"; $form.BackColor = "White"; $form.StartPosition = "CenterScreen"
    ...
    [void]$lv.Columns.Add("TÊN PHIÊN BẢN (GỒM ĐUÔI FILE)", 550); [void]$lv.Columns.Add("TRẠNG THÁI", 240); $form.Controls.Add($lv)

    # Khung chọn Thư mục lưu
    ...
    $txtPath.Text = Join-Path (Get-PSDrive -PSProvider FileSystem | Sort Free -Descending | Select -First 1).Root "VietToolbox_Downloads"
    ...
    # --- [2] LOGIC TẢI FILE (ĐÃ FIX ĐUÔI FILE) ---
    $btnDownload.Add_Click({
        ...
        foreach ($item in $items) {
            # --- PHẦN FIX: TỰ ĐỘNG LẤY ĐUÔI FILE TỪ TÊN ---
            $rawFileName = $item.Text.Replace(" ", "_")
            # Nếu tên không chứa đuôi file phổ biến, mặc định thêm .iso
            ...
            # Link tải (Hỗ trợ Bypass Virus Scan của Google)
            ...
            try {
                ...
                if (-not $response.IsSuccessStatusCode) {
                    $item.SubItems[1].Text = "❌ Lỗi: $($response.StatusCode)"; continue
                }
    ...
    function Load-IsoList {
        try {
            ...
            $lblStatus.Text = "✅ Đồng bộ xong $($lv.Items.Count) mục!"
        } catch { $lblStatus.Text = "❌ Lỗi đồng bộ danh sách!" }
    }
